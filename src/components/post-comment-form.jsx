    ...
    return (
        <>
            <form className="post-comment-form" onSubmit={handleSubmit}>
                ...
                <textarea className="post-comment-input" name="body" onChange={(e) => {
                    handleChange("body", e.target.value);
                }}></textarea>
                <input className="comment-button" type="submit" value="Post"></input>
            </form>
            { isPosted ? <section className="comment-card">
                    <p>Comment successfully posted!</p>
                    <p>{postedComment.username}: {postedComment.body}</p>
                    <p>Votes: 0 </p>
                    <p>&lt;1 minute ago</p></section> : <p>{err}</p>}
        </>
    )
}